# go-z3 の制約条件の DSL を考える

DSL とは Domain Specific Languageの略であり「ドメイン固有言語」を指す。

ここでは、SMT Solver の入力となる制約条件を記述するための DSL を考える。

基本方針：

* 新しい言語は golang と親和性のある構文仕様としたい。
* 新たにパーサを実装することはしたくない。
* 標準パッケージのみを使う。
...
次のような条件式を満たす整数 x と y を求めるサンプルコード。
...
制約条件の記述に集中したいのに、無駄な記述が多すぎる。

z3 のインポート、コンテクストやソルバーの作成の部分、制約変数の定義、そして、解決結果の表示の箇所はなるべく記述量を減らし簡潔にしたい。
