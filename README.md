...
conv コマンドの中で行っている、制約条件式の AST の変換とは次のような木構造を変換することとなる。
...
こうして得られたコードのパージング処理には go の標準パッケージである go/parser パッケージを利用する。
このようにしてパージング処理によって AST を取得する。
...
それぞれのケースに応じて switch 文で分岐していく。このあたりは AST ノードを
トラバースしていくだけだが、変換を必要としない式もあることに注意。
...
対応する AST ノードを生成することで式を変換する。また、式は再帰的な構造をしているため、演算子の引数となる式もまた再帰的に変換を行うことになる。
...
表示する制約変数の指定時に引用符をつけなくてもよいようにした。
