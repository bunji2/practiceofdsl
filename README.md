...
次のような条件式を満たす整数 x と y を求める簡単なサンプルコードを示す。
...
SMT Solver のユーザとしてみれば、制約条件の記述に集中したいだけなのに。

というわけで、SMT Solver に渡す制約条件を記述するためだけの専用言語を考えてみたい。

まずは、上のサンプルコードの少々雑多な部分をライブラリ化してみる。
